...
vis_seq_logo = function(sa_traj) {
  ...
  frags = vector(mode = "character", length = ncol(sa_traj));
  for (i in 1:ncol(sa_traj)) {
...
split_sa_align = function(sa_traj, n_block) {
  ## number of rows in trajectory
  n_row = nrow(sa_traj);
  ## number of blocks with full row number
  n_block_full = floor(n_row / as.integer(n_block));
  ## number of rows in each block
  n_block_rows = n_row / n_block_full;

  ## initialise list to append into
  sa_list = list();
  ## generate vector dereferencing the start row for each block in the alignment
  ref_seq = seq(from = 1, to = (n_block_full * n_block_rows), by = n_block_row);

  ## loop over the structural alphabet alignment
  for (i in c(1:length(ref_seq))) {
    rowstart = ref_seq[i];
    rowend = ref_seq[i] + (n_block_row - 1);

    ## assign block to temporary matrix
    sa_mat = sa_traj[c(rowstart : rowend), ];
