...
plot.matrices = function(){
	...
	for (x in 1:length(list.filenames)){
		...
		list.data[[x]][is.na(list.data[[x]])] <- 0
	}
...
eigen.cosine = function(matrix.a, matrix.b){
	...
	d.ab = (sum((eigen.a$vectors %*% eigen.b$vectors)^2)/
		ncol(matrix.a))
...
covariance.overlap = function(matrix.a, matrix.b){
	...
	omega.values = sum(eigen.a$values + eigen.b$values)	
	
	upper.omega.vec = 2 * sum((sqrt(eigen.a$values * eigen.b$values)) *
				(eigen.a$vectors %*% eigen.b$vectors)^2)
